...
namespace coen79_lab4
{
    ...
    public:
        ...
        //constructors
        keyed_bag();//doesn't initialize data[] or keys[]
        
        //member functions
        void erase();// just sets used to 0
        bool erase(const key_type& key);//
        void insert(const value_type& entry, const key_type& key);//
        void operator +=(const keyed_bag& addend);
    ...
    private:
        value_type data[CAPACITY];//array used to store the data
        size_type used;
        key_type keys[CAPACITY];//array used to store the keys
